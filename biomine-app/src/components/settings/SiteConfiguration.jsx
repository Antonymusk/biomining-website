...
export default function SiteConfiguration() {
  ...
  const filteredSites = useMemo(() => {
    return sites.filter(site => 
      ...
      (site.manager && site.manager.toLowerCase().includes(search.toLowerCase())) ||
      (site.status && site.status.toLowerCase().includes(search.toLowerCase()))
    );
  }, [sites, search]);
  ...
  const handleOpenModal = (site = null) => {
    ...
    if (site) {
      ...
      setEditingSite(null);
      setFormData({ name: "", zone: "Central", capacity: "500", status: "Active", hours: "24 Hours", manager: "" });
    }
    setIsModalOpen(true);
  };
    ...
    <div className="space-y-6 h-full flex flex-col">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">Register new facilities and manage active governance per site.</p>
        </div>
        {!isReadOnly && (
      ...
      <Card className="flex-1 flex flex-col overflow-hidden p-0">
        <div className="p-4 border-b border-white/5 flex items-center gap-4 bg-slate-950/20">
          <div className="relative flex-1 max-w-md">
            ...
            <input 
              type="text"
              placeholder="Search via Site Name, Status or Incharge Name..."
              value={search}
          ...
          <div className="hidden md:flex items-center gap-2 px-3 py-2 bg-slate-900/50 rounded-lg border border-white/5 text-xs text-slate-400">
            <Building2 size={14} />
            Active Facilities: {sites.length}
          </div>
        </div>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="text-left text-[10px] tracking-wider text-slate-400 uppercase bg-slate-900/30 border-b border-white/5">
                ...
                <th className="px-6 py-3 font-semibold">Site Incharge Name</th>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-md glass-card border border-white/10 shadow-2xl rounded-2xl overflow-hidden flex flex-col"
            >
              <div className="flex justify-between items-center p-6 border-b border-white/5 bg-slate-900/20">
                <h3 className="text-lg font-bold text-white flex items-center gap-2">
                  ...
                  {editingSite ? "Edit Site Configuration" : "Add New Site"}
                </h3>
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4 overflow-y-auto">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Site Incharge Name</label>
                  <input 
